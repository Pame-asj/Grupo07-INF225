...
from AutoridadDocente.models import Ensayo, RespuestaEnsayo, RespuestaPregunta, Pregunta, Tema
...
def ensayos_view(request):
    ...
    ensayos = Ensayo.objects.filter(colegio=estudiante.colegio)
    ...
    # Aplicar filtros
    if tema_id:
        ensayos = ensayos.filter(temas__id=tema_id)
    if nivel:
        ensayos = ensayos.filter(nivel=nivel)
    
    # Obtener temas disponibles para los filtros (solo temas usados en ensayos del estudiante)
    temas_disponibles = Tema.objects.filter(ensayo__in=ensayos).distinct()
    ...
    context = {
        ...
        'filtros_aplicados': tema_id or nivel  # Para mostrar el botón de limpiar
    }
    return render(request, 'ensayos.html', context)
